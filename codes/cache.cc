...
void CACHE::bandwidth_calc(uint32_t cpu, CACHE *cache)
{       
		...
		if (RQ.entry[index].type == LOAD) {
			...
			cpu = 0;
                ...
                if (cache_type == IS_LLC) 
                {
                    ...
                    // WaZ -------------------------
                    block[set][way].uCounter++;
                    // uint64_t block_addr = block[set][way].address;
                    uint64_t block_full_addr = block[set][way].full_addr;
                    
                    map<uint32_t, uint64_t> tmp_map;
                    tmp_map[read_cpu] = block_full_addr;
                    BLOCK_ACCESS_cpu[tmp_map] = block[set][way].uCounter;   // stores (cpu,full_addr) -> count           
                    // -----------------------------
                }
                else
		  ...
		  if(fill_level == FILL_L2)
		  ...
		  else
		    {
		      ...
		      if (RQ.entry[index].is_data)
                ...
                // BLOCK_ACCESS[block_addr] = block[set][way].uCounter;                  // stores (block_addr) -> count     
                // BLOCK_ACCESS_full_addr[block_full_addr] = block[set][way].uCounter;   // stores (full_addr) -> count             
                // --------------------------------------------------------------------
...
// --------------------------------------
vector<uint32_t> generate_RBM(uint32_t rows, uint32_t cols)
{
  vector<uint32_t> rbm;
...
vector<uint32_t> create_nBit_binary(uint64_t x, uint32_t n)
{
...
uint32_t convert_decimal_upto_nBits(vector<uint32_t> x, uint32_t n)
{
  uint32_t decimal = 0;
  uint32_t base = 1;
  uint32_t start = x.size()-1;
  uint32_t end = x.size()-n;
...
uint32_t random_hash(uint32_t x, uint32_t x_size)
{
  vector<uint32_t> rbm = generate_RBM(LOG2_NUM_CLUSTERS, x_size);
  vector<uint32_t> hash_value_bits;

  int rbm_size = rbm.size();

  for(int i=0; i<rbm_size; i++)
  {
    rbm[i] = rbm[i] & x;
    vector<uint32_t> x_bit = create_nBit_binary(rbm[i], x_size);
    uint32_t n =x_size/2;

    for(int j=0; j<3; j++)
    {
      vector<uint32_t> tmp_xBit;

      for(int k=0; k<n; k++)
        tmp_xBit.push_back(x_bit[2*k] ^ x_bit[(2*k)+1]);
      
      n /= 2;
      
      int tmp_xBit_size = tmp_xBit.size();
      for(int k=0; k<tmp_xBit_size; k++)
        x_bit[k] = tmp_xBit[k];
    }
    hash_value_bits.push_back((x_bit[0] ^ x_bit[1]) ^ x_bit[2]);
  }
  uint32_t hash_value = convert_decimal_upto_nBits(hash_value_bits, hash_value_bits.size() - 1);
  return hash_value;
}

uint32_t CACHE::fetch_DBT(uint32_t cpu)
{
  return DBT[cpu];
}

uint32_t CACHE::fetch_CIM(uint32_t LCID, uint32_t LCID0)
{
  return CLT[ LCID0 + LCID ];
}

uint32_t CACHE::LBH(vector<uint32_t> x, uint32_t num_of_clusters, uint32_t n)
{
  vector<uint32_t> LCID;

  if( convert_decimal_upto_nBits(x, n+1) < num_of_clusters )
  {
  ...
  else
  {
    ...
    for(int i=0; i<5; i++)
    {
      uint32_t hash_value = random_hash(convert_decimal_upto_nBits(x, x.size()), x.size());
      if(hash_value < num_of_clusters)
      {
        flag = true;
        LCID = create_nBit_binary(hash_value, n);
        break;
      }
    }
  ...
  uint32_t final_LCID = convert_decimal_upto_nBits(LCID, LCID.size());
  
  return final_LCID;

}

uint32_t CACHE::CIM(uint32_t cpu, uint32_t LCID, vector<uint32_t> line_address)
{
  uint32_t LCID0 = fetch_DBT(cpu);
  uint32_t PCID = fetch_CIM(LCID, LCID0);
  vector<uint32_t> cluster_offset = vector<uint32_t>(line_address.end() - 6, line_address.end());
  
  uint32_t cluster_offset_ = convert_decimal_upto_nBits(cluster_offset, cluster_offset.size());
...
uint32_t CACHE::get_set(uint64_t address, uint32_t cpu)
{
  if(cache_type == IS_LLC)
  {
    ...
    vector<uint32_t> line_address = create_nBit_binary(address, 48);
    
    vector<uint32_t> x = vector<uint32_t> (line_address.end() - 30, line_address.end() - 6);

    uint32_t LCID = LBH(x, num_of_clusters, n);

    uint32_t SET_ID = CIM(cpu, LCID, line_address);
