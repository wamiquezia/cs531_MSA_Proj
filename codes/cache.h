...
class CACHE : public MEMORY {
  public:
    ...
    map<uint32_t, uint32_t> SET_ACCESS;
    
    map<uint64_t, uint32_t> BLOCK_ACCESS;
    map<map<uint32_t, uint64_t>, uint32_t> BLOCK_ACCESS_cpu;
    map<uint64_t, uint32_t> BLOCK_ACCESS_full_addr;
    // uint32_t TOTAL_SET_REF;
    // ---------------------------------------------
    ...
    uint32_t get_set(uint64_t address, uint32_t cpu),   // WaZ
    ...
    // WaZ ------------------------------
    uint32_t fetch_DBT(uint32_t cpu),
             fetch_CIM(uint32_t LCID, uint32_t LCID0),
             LBH(vector<uint32_t> x, uint32_t num_of_clusters, uint32_t n),
             CIM(uint32_t cpu, uint32_t LCID, vector<uint32_t> line_address);

    // num_of_blocks_to_prefetch
    void bandwidth_calc(uint32_t cpu, CACHE *cache);
    // ----------------------------------
};
